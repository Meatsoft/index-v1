# app.py — LaSultana Meat Index (2 pechugas + cinta USD + bordes corregidos)
...
/* ==== Tabla pollo (un solo marco, sin borde interno) ==== */
.card.pechugas{
  border:1px solid var(--line);
  border-radius:12px;
  overflow:hidden;          /* recorta cualquier esquina interna */
  padding:0;
  margin-bottom:18px;
}
.pechugas .panel{padding:0;border:0;border-radius:0}

.pechugas table{width:100%;border-collapse:separate;border-spacing:0}
.pechugas th,.pechugas td{
  padding:12px 14px;
  vertical-align:middle;
  border-bottom:1px solid var(--line);  /* solo líneas horizontales */
  border-left:0; border-right:0;        /* sin paredes laterales extra */
...
/* Noticias (22% más rápida vs 150s -> 117s) */
...
fx,fx_chg = get_fx()
lc,lc_chg = get_last("LE=F")   # Live Cattle (USD/100lb)
lh,lh_chg = get_last("HE=F")   # Lean Hogs  (USD/100lb)

def kpi(title, price, chg, unit="USD/100 lb"):
    if title=="USD/MXN":
        unit=""  # sin unidad extra
    if price is None:
        price_html=f"<div class='big'>N/D {'<span class=\"unit-inline\">'+unit+'</span>' if unit else ''}</div>"; delta_html=""
    else:
        if chg is None:
            price_html=f"<div class='big'>{fmt2(price)} {'<span class=\"unit-inline\">'+unit+'</span>' if unit else ''}</div>"; delta_html=""
        else:
            cls="up" if chg>=0 else "down"; arr="▲" if chg>=0 else "▼"
...
st.markdown(kpi("USD/MXN", fx, fx_chg, unit=""), unsafe_allow_html=True)
...
<div class="card pechugas">
  <div class="panel">
    <div class="title" style="color:var(--txt);margin:12px 14px 6px">
...
# refresco simple (puede parpadear levemente en Streamlit Cloud)
time.sleep(60)
st.rerun()
