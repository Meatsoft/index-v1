# app.py — LaSultana Meat Index (Banxico FIX para USD/MXN, sin parpadeo)
# - Bursátil (yfinance)
# - USD/MXN (Banxico FIX SF43718) con snapshot local (sin inventar)
...
# ==================== USD/MXN desde BANXICO (FIX) ====================
BANXICO_TOKEN = os.getenv("BANXICO_TOKEN", "").strip()
BANXICO_SERIES = "SF43718"  # FIX
FX_SNAPSHOT = "fx_last.json"

def _load_fx_snap():
    if not os.path.exists(FX_SNAPSHOT): return None
    try:
        with open(FX_SNAPSHOT, "r") as f: return json.load(f)
    except: return None

def _save_fx_snap(value: float, date_str: str):
    try:
        with open(FX_SNAPSHOT, "w") as f:
            json.dump({"value": float(value), "date": date_str}, f)
    except: pass

def get_fx_banxico():
    """Devuelve (valor, delta_vs_prev, fecha, usado_snapshot: bool). Sin inventar."""
    prev = _load_fx_snap()
    if BANXICO_TOKEN:
        try:
            ...
            j = r.json()
            datos = j["bmx"]["series"][0]["datos"][0]
            fecha = datos["fecha"]
            valor = float(datos["dato"])
            # delta vs snapshot anterior (si existe)
            delta = 0.0
            if prev and "value" in prev:
                try: delta = valor - float(prev["value"])
                except: delta = 0.0
            _save_fx_snap(valor, fecha)
            return valor, delta, fecha, False
        except Exception:
            pass
    # Fallback: snapshot local
    if prev and "value" in prev:
        try:
            return float(prev["value"]), 0.0, prev.get("date",""), True
        except: pass
    # Primer arranque sin token y sin snapshot
...
# ===== Render helpers =====
def render_fx(ph, fx_val, fx_delta, fx_date, used_snapshot):
    # Etiqueta clara: Banxico FIX
    title = "USD/MXN (Banxico FIX)"
    badge = " <span class='badge'>último disponible</span>" if used_snapshot else ""
    if fx_val is None:
        price_html = f"<div class='big'>N/D <span class='unit-inline'>FIX</span></div>"
        delta_html = ""
    else:
        cls = "green" if (fx_delta or 0) >= 0 else "red"
        arr = "▲" if (fx_delta or 0) >= 0 else "▼"
        price_html = f"<div class='big'>{fmt4(fx_val)} <span class='unit-inline'>FIX</span></div>"
        delta_html = f"<div class='delta {cls}'>{arr} {fmt4(abs(fx_delta or 0))}</div>"
    ph.markdown(f"""
    <div class="card">
      <div class="kpi">
        <div class="left">
          ...
          {price_html}
          <div class="muted" style="font-size:12px;margin-top:4px;">{fx_date}</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
while True:
    try:
        # Banxico FIX (stable daily). Muestra snapshot si no hay token o no responde.
        fx_val, fx_delta, fx_date, fx_snapshot = get_fx_banxico()
        ...
        render_fx(fx_ph, fx_val, fx_delta, fx_date, fx_snapshot)
        ...
        footer_ph.markdown(
            f"<div class='caption'>Actualizado: {dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} · Auto-refresh 60s · Fuentes: Banxico (FIX) · USDA · USMEF · Yahoo Finance (~15 min retraso).</div>",
            unsafe_allow_html=True,
        )
    except Exception:
        pass
