# app.py — LaSultana Meat Index (2 pechugas + cinta USD, bordes redondeados y USDA robusto)
...
try:
    # limpia cache en cada deploy; en runtime dejamos que cachee
...
try:
    ...
    pass  # si no está instalado, igual funciona
...
/* oculta spinner/cambios para que no parpadee */
...
/* Tabla Pechugas (2) con esquinas inferiores redondeadas */
.table-shell{border:1px solid var(--line);border-radius:10px;overflow:hidden;background:var(--panel)}
.pech table{width:100%;border-collapse:separate;border-spacing:0}
.pech th,.pech td{padding:12px 10px;border-bottom:1px solid var(--line);vertical-align:middle}
.pech th{text-align:left;color:var(--muted);font-weight:700;letter-spacing:.2px}
.pech td:first-child{font-size:112%}
.pech tr:last-child td{border-bottom:0} /* sin línea extra al final */
...
/* Noticias (22% más rápido vs 150s -> 117s) */
...
COMPANIES = [
    ...
    ("Walmart","WMT"),
    # (Se removieron BAFARB.MX y ALSEA.MX por MXN/actividad)
]
...
# ======= USDA — Pechuga B/S y Pechuga T/S (strapless) =======
POULTRY_URLS = [
...
PECH_PAT = {
    "Pechuga B/S":[
        r"BREAST\s*[-,]\s*B/?S\b",
        r"BONELESS\s*SKINLESS\s*BREASTS?",
        r"BREASTS?\s*\(B/?S\)"
    ],
    "Pechuga T/S (strapless)":[
        r"BREAST\s*T/?S\b",
        r"STRAPLESS\s*BREASTS?"
    ],
}
...
def _avg(line_up:str):
    ...
    # último número con decimales como precio
...
def fetch_usda_latest(days_back:int=7) -> dict:
    today = dt.date.today()
    # 1) sin fecha (suele devolver el último disponible)
    ...
    # 2) con fechas hacia atrás (7 días)
...
def pechugas_with_snapshot():
    ...
    # sin nada (primer run sin red): placeholders
