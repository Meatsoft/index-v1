# app.py — LaSultana Meat Index (V1.6.1 fix: no code-blocks, rotador limpio)
import os, time, re, datetime as dt, textwrap as tw
...
/* Industry Monitor: 3 slots (10s c/u), sin superposición */
...
for country, badge in [("Estados Unidos","USA"),("Brasil","BRA"),("México","MEX")]:
    st.markdown(tw.dedent(f"""
    <div class="panel">
      <div class="head">{country} <span class="badge">{badge}</span></div>
      <div><span class="okdot"></span>Sin novedades significativas (última revisión reciente).</div>
    </div>
    """), unsafe_allow_html=True)
st.markdown("</div></div>", unsafe_allow_html=True)

# ==================== FROZEN MEAT INDUSTRY MONITOR ====================
...
def signal_html(pct: float | None, label: str) -> str:
    if pct is None:
        ...
        num_txt = f"{pct:+.1f}%"; cls = "up" if pct >= 0 else "down"
    # Importante: sin indentación inicial para que Markdown NO lo trate como code-block.
    return (
        f"<div class='im-item'>"
        f"  <div class='im-num {cls}'>{num_txt}</div>"
...
signals = [signal_html(sig_fx,"USD/MXN"), signal_html(sig_lc,"Res (LE=F)"), signal_html(sig_lh,"Cerdo (HE=F)")]
signals = signals[:3] or ["<div class='im-item'><div class='im-num'>—</div><div class='im-sub'>Sin datos</div><div class='im-desc'>—</div></div>"]
while len(signals) < 3:
